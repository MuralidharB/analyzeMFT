...
class MFTTestGenerator:
    ...
    def create_filename_attribute(self, filename: str, parent_ref: int = 5) -> bytes:
        ...
        fn_data += struct.pack('<QQQQ',
            self.windows_time(now),            self.windows_time(now),            self.windows_time(now),            self.windows_time(now),        )
        fn_data += struct.pack('<QQII',
            file_size,            file_size,            FILE_ATTRIBUTE_NORMAL,            (filename_length << 8) | FILE_NAME_WIN32        )
        fn_data += filename_bytes
        attr_header = struct.pack('<IBBHHHH',
            FILE_NAME_ATTRIBUTE,            len(fn_data) + 24,            0,            0,            24,            0,            0        )
    ...
    def create_data_attribute(self, size: int = 0) -> bytes:
        ...
        if size == 0:
            attr_header = struct.pack('<IBBHHHH',
                DATA_ATTRIBUTE,                24,                0,                0,                24,                0,                0            )
            return attr_header
        else:
            attr_header = struct.pack('<IBBHHHH',
                DATA_ATTRIBUTE,                64,                1,                0,                64,                0,                0            )
            nr_data = struct.pack('<QQHHHH',
                0,                (size + 4095) // 4096 - 1,                64,                0,                0,                0            )
        ...
        with open(output_path, 'wb') as f:
            if include_system_files:
                ...
                for i in range(16):
                    ...
                    else:                        record = b'\x00' * MFT_RECORD_SIZE
                    f.write(record)
