...
class MFTValidationError(ValidationError):
    """Raised when MFT file validation fails."""
    pass
...
class NumericValidationError(ValidationError):
    """Raised when numeric parameter validation fails."""
    pass
...
def validate_mft_file(file_path: str) -> Path:
    """
    Validate MFT file exists, is accessible, and has correct format.
...
def validate_output_path(output_path: str) -> Path:
    ...
    Returns:
        Path: Resolved and validated output path
...
def validate_numeric_bounds(chunk_size: int, hash_processes: Optional[int] = None, 
    ...
    """
    Validate all numeric parameters are within safe bounds.
    
    Args:
        chunk_size: Number of records per processing chunk
        hash_processes: Number of hash computation processes (optional)
        test_records: Number of test records to generate
        verbosity: Verbosity level
        debug: Debug level
        
    Raises:
        NumericValidationError: If any parameter is out of bounds
    """
...
def validate_export_format(export_format: str, output_file: str) -> str:
    """
    Validate export format and check compatibility with output file.
    ...
    Returns:
        str: Validated export format
        
    Raises:
        ValidationError: If format is invalid or incompatible
    """
...
def validate_attribute_length(attr_len: int, offset: int, record_size: int, attr_type: int = None) -> None:
    """
    Validate MFT attribute length to prevent buffer overruns.
    
    Args:
        attr_len: Attribute length from MFT record
        offset: Current offset in record
        record_size: Total size of MFT record
        attr_type: Attribute type (for logging)
        
    Raises:
        ValidationError: If attribute length is invalid
    """
    if attr_len <= 0:
...
def validate_config_schema(config: Dict[str, Any]) -> Dict[str, Any]:
    """
    Validate configuration file schema and values.
...
def validate_paths_secure(input_file: str, output_file: str) -> Tuple[Path, Path]:
    ...
    Args:
        input_file: Input MFT file path
        output_file: Output file path
